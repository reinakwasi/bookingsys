  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 relative overflow-hidden">
      ...
      <section className="section-responsive relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-50/30 via-white to-slate-50" />
        <div className="container-responsive relative z-10">
          <div className="text-center mb-12 sm:mb-16 lg:mb-20 animate-fade-in-up">
            <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2 sm:py-3 rounded-full glass-morphism text-[#C49B66] mb-6 sm:mb-8 animate-glow">
              <Calendar className="h-4 w-4 sm:h-5 sm:w-5 animate-pulse" />
              <span className="text-sm sm:text-base font-medium">Upcoming Events</span>
              <CalendarDays className="h-4 w-4 sm:h-5 sm:w-5 animate-pulse" />
            </div>
            <h2 className="text-responsive-3xl font-display font-bold mb-6 sm:mb-8 text-gradient">Hotel Events</h2>
            <p className="text-responsive-base text-slate-600 max-w-3xl mx-auto leading-relaxed">
              Discover our upcoming hotel events and experiences. Reserve your spot today.
            </p>
          </div>
          ...
          {upcomingEvents.length > 0 ? (
            <div className={`grid gap-6 sm:gap-8 lg:gap-12 ${
              ...
              {upcomingEvents.map((event, index) => (
                <div key={event.id} className="group animate-fade-in-up" style={{animationDelay: `${index * 0.2}s`}}>
                    ...
                    <div className="relative h-80 overflow-hidden">
                      ...
                      <div className="absolute top-6 left-6 z-10">
                        <div className="glass-morphism rounded-2xl px-4 py-2 border border-white/20">
                          <span className="text-white font-bold text-sm">{event.category}</span>
                        </div>
                      </div>
            ...
            <div className="text-center animate-fade-in-up">
              <div className="glass-morphism rounded-2xl sm:rounded-3xl p-8 sm:p-12 lg:p-16 shadow-luxury max-w-2xl mx-auto">
                <div className="gradient-gold rounded-full w-24 h-24 sm:w-32 sm:h-32 flex items-center justify-center mx-auto mb-6 sm:mb-8 animate-float">
                  <AlertCircle className="h-12 w-12 sm:h-16 sm:w-16 text-white" />
                </div>
                <h3 className="text-responsive-2xl font-display font-bold mb-4 sm:mb-6 text-slate-800">No Upcoming Events</h3>
                <p className="text-responsive-base text-slate-600 leading-relaxed mb-6 sm:mb-8">
                  We're currently planning some amazing events for you. Stay tuned for exciting announcements!
                </p>
      ...
      {featuredTickets.length > 0 && (
        <section className="section-responsive relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-amber-50/30 via-white to-yellow-50" />
          <div className="container-responsive relative z-10">
            <div className="text-center mb-12 sm:mb-16 lg:mb-20 animate-fade-in-up">
              <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2 sm:py-3 rounded-full glass-morphism text-amber-600 mb-6 sm:mb-8 animate-glow">
                <Ticket className="h-4 w-4 sm:h-5 sm:w-5 animate-pulse" />
                <span className="text-sm sm:text-base font-medium">Hotel Experiences</span>
                <Ticket className="h-4 w-4 sm:h-5 sm:w-5 animate-pulse" />
              </div>
              <h2 className="text-responsive-3xl font-display font-bold mb-6 sm:mb-8 text-gradient">Exclusive Activities</h2>
              <p className="text-responsive-base text-slate-600 max-w-3xl mx-auto leading-relaxed">
                Book your spot for these exclusive hotel activities and unforgettable experiences.
              </p>
            </div>

            <div className="grid grid-responsive-3 gap-6 sm:gap-8">
              {featuredTickets.map((ticket, i) => (
                <div key={ticket.id} className="group animate-fade-in-up" style={{animationDelay: `${i * 0.2}s`}}>
                  <div className="glass-morphism rounded-3xl overflow-hidden shadow-luxury hover:shadow-glow transition-all duration-500 transform hover:-translate-y-4 hover:scale-105">
                    {ticket.image_url && (
                      <div className="relative h-64 overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4 z-10">
                          <div className="glass-morphism rounded-2xl px-3 py-1 border border-white/20">
                            <span className="text-white font-semibold text-sm">{ticket.activity_type}</span>
                          </div>
                        </div>
                        
                        {/* Price Badge */}
                        <div className="absolute top-4 right-4 z-10">
                          <div className="bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-900 font-bold text-lg px-4 py-2 rounded-2xl shadow-luxury animate-float border border-white/20">
                            GHC {ticket.price}
                          </div>
                        </div>
                        ...
                        <div className="absolute bottom-4 left-4 right-4">
                          <div className="glass-morphism rounded-2xl p-3 border border-white/20">
                            <div className="flex items-center gap-3 text-white text-sm">
                              <div className="flex items-center gap-1">
                                <Calendar className="h-4 w-4 text-amber-400" />
                                <span>{new Date(ticket.event_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <Clock className="h-4 w-4 text-amber-400" />
                                <span>{new Date(`2000-01-01T${ticket.event_time}`).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="p-6">
                      <h3 className="text-2xl font-display font-bold mb-3 text-slate-800 group-hover:text-amber-600 transition-colors">
                        {ticket.title}
                      </h3>
                      <p className="text-slate-600 leading-relaxed mb-4 line-clamp-2">{ticket.description}</p>
                      
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2 text-slate-600">
                          <Users className="h-4 w-4 text-amber-600" />
                          <span className="text-sm">{ticket.available_quantity || ticket.total_quantity} spots left</span>
                        </div>
                        {ticket.venue && (
                          <div className="flex items-center gap-1 text-slate-600">
                            <MapPin className="h-4 w-4 text-amber-600" />
                            <span className="text-sm truncate max-w-24">{ticket.venue}</span>
                          </div>
                        )}
                      </div>
                      
                      <Link href="/tickets">
                        <Button className="w-full bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-900 font-semibold py-3 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-amber-500/30">
            ...
            <div className="text-center mt-8 sm:mt-12 animate-fade-in-up">
              <Link href="/tickets">
                <Button size="lg" className="bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-900 font-bold px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-amber-500/30">
